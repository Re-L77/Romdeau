...
const getEstadoConfig = (nombre: string | null) => {
  ...
  if (n.includes('BUENO') || n.includes('NUEVO'))
    return { bg: 'bg-emerald-100 dark:bg-emerald-500/20', text: 'text-emerald-700 dark:text-emerald-400', border: 'border-emerald-200 dark:border-emerald-700/30', icon: CheckCircle2 };
  if (n.includes('MALO') || n.includes('DA'))
    return { bg: 'bg-amber-100 dark:bg-amber-500/20', text: 'text-amber-700 dark:text-amber-400', border: 'border-amber-200 dark:border-amber-700/30', icon: AlertTriangle };
  if (n.includes('BAJA') || n.includes('NO'))
    return { bg: 'bg-red-100 dark:bg-red-500/20', text: 'text-red-700 dark:text-red-400', border: 'border-red-200 dark:border-red-700/30', icon: XCircle };
  return { bg: 'bg-gray-100 dark:bg-gray-800', text: 'text-gray-700 dark:text-gray-300', border: 'border-gray-200 dark:border-gray-700', icon: ClipboardList };
};

const formatFecha = (value: string | null) => {
  if (!value) return '—';
  try {
    return new Intl.DateTimeFormat('es-MX', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }).format(new Date(value));
  } catch { return value; }
};
...
function exportarCSV(logs: LogAuditoria[], periodo: string) {
  const escaparCsv = (v: string | null | undefined) => {
    const s = v ?? '';
  ...
  const csv = [...meta, encabezados.join(','), ...filas].join('\n');
  descargar(new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' }), `logs_auditoria_${hoy()}.csv`);
}

// ─── Excel SpreadsheetML nativo (sin advertencias de seguridad) ───────────────
function exportarExcel(logs: LogAuditoria[], periodo: string) {
  const celda = (val: string, styleId: string) =>
    `<Cell ss:StyleID="${styleId}"><Data ss:Type="String">${esc(val)}</Data></Cell>`;

  const filas = logs.map((l, i) => {
...
function filtrarPorPeriodo(logs: LogAuditoria[], periodo: Periodo, fechaInicio?: string, fechaFin?: string): LogAuditoria[] {
...
function ExportModal({ logs, onClose }: ExportModalProps) {
  ...
  const etiquetaPeriodo = periodo === 'custom' && fechaInicio && fechaFin
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
          <div>
            <h2 className="text-xl font-bold dark:text-white">Exportar Logs de Auditoría</h2>
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-6">
          ...
          <div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className={`font-medium text-sm ${activo ? 'text-emerald-700 dark:text-emerald-400' : 'text-gray-900 dark:text-white'}`}>{label}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{desc}</p>
                      </div>
                      {activo && <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />}
                    </div>
                  </button>
                );
              })}
            ...
            {periodo === 'custom' && (
              <div className="mt-3 grid grid-cols-2 gap-3 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                <div>
                  <label className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1 block">Fecha inicio</label>
                  <input
                    type="date" max={today} value={fechaInicio}
                ...
                <div>
                  <label className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1 block">Fecha fin</label>
                  <input
                    type="date" max={today} value={fechaFin}
          ...
          {/* ── Formato ────────────────────────────────────────── */}
          <div>
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Formato de archivo</p>
            <div className="grid grid-cols-1 gap-3">
              {formatoOpciones.map(({ value, label, desc, icon: Icon }) => {
                const activo = formato === value;
                return (
                  <button
                    key={value}
                    type="button"
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className={`font-semibold text-sm ${activo ? 'text-white dark:text-black' : 'text-gray-900 dark:text-white'}`}>{label}</p>
                      <p className={`text-xs mt-0.5 ${activo ? 'text-white/70 dark:text-black/60' : 'text-gray-500 dark:text-gray-400'}`}>{desc}</p>
                    </div>
                    {activo && <CheckCircle2 className="w-5 h-5 text-white dark:text-black flex-shrink-0" />}
                  </button>
                );
              })}
            </div>
          ...
          <div className="p-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-700/30 rounded-xl">
            <p className="text-sm font-semibold text-emerald-900 dark:text-emerald-400 mb-2">Resumen de exportación</p>
            <ul className="text-sm text-emerald-800 dark:text-emerald-300 space-y-1">
              <li>• Formato: <span className="font-semibold">{formato}</span></li>
              <li>• Período: <span className="font-semibold">{etiquetaPeriodo}</span></li>
              <li>• Registros a exportar: <span className="font-semibold">{logsExport.length}</span> {logsExport.length !== logs.length && <span className="text-emerald-600/70 dark:text-emerald-400/70">(de {logs.length} total filtrados)</span>}</li>
            </ul>
          </div>
        ...
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex-shrink-0">
          <p className="text-xs text-gray-400 dark:text-gray-600">El archivo se descargará automáticamente</p>
          <div className="flex gap-3">
            ...
            <button type="button" onClick={handleExport} disabled={exporting || logsExport.length === 0}
              ...
              {exporting ? <><Loader2 className="w-4 h-4 animate-spin" />Generando...</> : <><Download className="w-4 h-4" />Descargar {formato}</>}
            </button>
          </div>
        </div>
      </motion.div>
...
export function RegistroAuditorias({ onAuditClick }: RegistroAuditoriasProps) {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      const result = await logsAuditoriaApi.getList({ limit: 100 });
      setLogs(result.data);
    } catch (err: any) {
  ...
  const statuses = ['all', ...Array.from(new Set(logs.map((l) => l.estado_reportado).filter(Boolean)))] as string[];
  const auditors = ['all', ...Array.from(new Set(logs.map((l) => l.auditor).filter(Boolean)))] as string[];
    ...
    <main className="pl-6 lg:pl-80 pt-6 lg:pt-8 pb-12 px-6 pr-6 lg:pr-12">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="mb-8 mt-6">
          <h1 className="text-3xl font-bold mb-2 dark:text-white">Logs de Auditoría</h1>
        ...
        <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.5)] p-6 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <select value={auditorFilter} onChange={(e) => setAuditorFilter(e.target.value)}
              className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-800 dark:text-white border border-gray-200 dark:border-gray-700 rounded-full text-sm focus:outline-none focus:border-black dark:focus:border-white transition-colors">
              {auditors.map((a) => <option key={a} value={a}>{a === 'all' ? 'Todos los auditores' : a}</option>)}
            </select>
          </div>
        </div>
        ...
        {!loading && error && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-6 mb-4 flex items-center gap-3">
            ...
            <p className="text-red-700 dark:text-red-400 font-medium flex-1">{error}</p>
            <button onClick={fetchLogs} className="text-sm text-red-600 dark:text-red-400 underline hover:no-underline">Reintentar</button>
          </div>
        )}
        ...
        {!loading && !error && filteredLogs.length === 0 && (
          <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.5)] p-16 flex flex-col items-center gap-4">
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium">No se encontraron registros de auditoría</p>
          </div>
        )}
        ...
        {!loading && !error && filteredLogs.length > 0 && (
          <div className="space-y-3">
            {filteredLogs.map((log, index) => {
              const estadoConfig = getEstadoConfig(log.estado_reportado);
                ...
                <motion.div key={log.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }}
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">{log.activo?.nombre ?? 'Sin nombre'}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <p className="text-xs text-gray-700 dark:text-gray-300">{formatFecha(log.fecha_hora)}</p>
                      </div>
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${estadoConfig.bg} ${estadoConfig.text} ${estadoConfig.border}`}>
      ...
      {isExportModalOpen && (
        <ExportModal logs={filteredLogs} onClose={() => setIsExportModalOpen(false)} />
      )}
    </main>
  );
}