...
export function Alertas() {
  ...
  const { user } = useAuth();
  const [modalType, setModalType] = useState<'acquisition' | 'bookValue' | 'depreciation' | 'warranties' | null>(null);

  if (loading || !summary) return <div className="p-20 text-center animate-pulse text-gray-400">Generando reporte financiero...</div>;
  ...
  const exportDevaluationToPDF = () => {
    ...
    doc.setFontSize(10);
    doc.setTextColor(80);
    const userName = user ? `${user.nombres || ''} ${user.apellido_paterno || ''} ${user.apellido_materno || ''}`.trim().replace(/\s+/g, ' ') : 'Usuario Desconocido';
    doc.text(`Generado por: ${userName}`, 14, 30);
    doc.text(`Fecha de Emisión: ${new Date().toLocaleDateString('es-MX')} a las ${new Date().toLocaleTimeString('es-MX', {hour: '2-digit', minute:'2-digit'})}`, 14, 36);
    doc.text(`Total Activos Controlados: ${summary.totalActivos}`, 14, 42);

    const headers = ["ID", "Activo", "Categoría", "Fecha de Adq.", "Vida Útil", "Costo Original", "Depreciación", "Valor en Libros", "Desgaste", "Estado"];
    const rows: string[][] = [];

    summary.activosDetalle.forEach((activo: any) => {
      const pct = ((activo.depreciacionAcumulada / activo.valorAdquisicion) * 100).toFixed(1);
      const isFullyDepreciated = parseFloat(pct) >= 100 || activo.valorLibro <= 0;
      
      const rowId = activo.id.includes('-') ? activo.id.split('-')[1] : activo.id;
  ...
  const calculatedPercentage = ((summary.depreciacionAcumulada / summary.valorTotalAdquisicion) * 100).toFixed(1);
...
=======
    <main className="pl-6 lg:pl-80 pt-6 lg:pt-8 pb-12 px-6 pr-6 lg:pr-12 print:p-0 print:m-0">
...
>>>>>>> sofia
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Análisis detallado de la valoración patrimonial, desgaste de infraestructura y estado de cobertura legal.
            </p>
          </div>
          ...
          <div className="flex gap-4">
            <div className="p-4 bg-white dark:bg-[#1a1a1a] rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.5)] w-fit border border-gray-100 dark:border-white/5">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Total Activos Auditados</p>
              <p className="text-2xl font-black text-gray-900 dark:text-white">{summary.totalActivos}</p>
            </div>
          </div>
        </motion.div>
          ...
          {kpis.map((kpi, i) => (
            ...
            >
              <div className="relative z-10">
                <div className={`w-14 h-14 ${kpi.bg} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500`}>
                ...
                <p className="text-3xl font-black text-gray-900 dark:text-white mb-2">{kpi.value}</p>
                {kpi.subtext && <p className="text-xs font-bold text-gray-500">{kpi.subtext}</p>}
              </div>
        ...
        >
          <div className="p-8 md:p-10 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-black/20 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="text-xl font-black text-gray-900 dark:text-white tracking-tight">Reporte Contable Detallado</h4>
                <p className="text-sm text-gray-500 font-bold">Análisis de depreciación activo por activo</p>
              </div>
            </div>
            ...
            <table className="w-full text-left border-collapse min-w-[1000px]">
              <thead>
                <tr className="bg-gray-50/50 dark:bg-white/[0.02]">
                  <th className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 dark:border-white/5">Nombre del Activo</th>
                  <th className="px-6 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 dark:border-white/5">Categoría</th>
                  <th className="px-6 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 dark:border-white/5 text-center">Adquisición</th>
                  <th className="px-6 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 dark:border-white/5 text-center">Vida Útil</th>
                  <th className="px-6 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 dark:border-white/5 text-right">Valor Inicial</th>
                  <th className="px-6 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 dark:border-white/5 text-right">Dep. Acumulada</th>
                  <th className="px-6 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 dark:border-white/5 text-right">Valor Libros</th>
                  <th className="px-6 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 dark:border-white/5 text-center">% Dep.</th>
                  <th className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 dark:border-white/5 text-center">Estado</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                {summary.activosDetalle.map((activo: any) => {
                  ...
                  return (
                    <tr key={activo.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors group">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-gray-100 dark:bg-white/5 rounded-lg flex items-center justify-center text-[10px] font-bold text-gray-400">{activo.id.split('-')[1]}</div>
                          <p className="text-sm font-bold text-gray-900 dark:text-white group-hover:text-blue-500 transition-colors">{activo.nombre}</p>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-6 text-center">
                        <div className="flex flex-col items-center">
                          ...
                          <span className="text-xs font-bold text-gray-600 dark:text-gray-400">{new Date(activo.fechaAdquisicion).toLocaleDateString()}</span>
                        </div>
                      </td>
                      <td className="px-6 py-6 text-center">
                        <span className="px-2 py-1 bg-gray-100 dark:bg-white/5 rounded-md text-[10px] font-black text-gray-500">{activo.vidaUtil} Años</span>
                      </td>
