...
export function AssetInventory({ onAssetClick, onCreateAsset }: AssetInventoryProps) {
  ...
  const [confirmModal, setConfirmModal] = useState<{
    isOpen: boolean;
    action: 'create' | 'edit' | 'transfer' | 'delete' | 'editSpecs' | 'printLabel' | 'changeCustodian' | null;
    assetId?: string;
    assetName?: string;
  ...
  // Filter assets
  const filteredAssets = activosEnriquecidos.filter(asset => {
    if (categoryFilter !== 'all' && asset.categoria?.nombre !== categoryFilter) return false;
    if (statusFilter !== 'all' && asset.estado_operativo_id !== statusFilter) return false;
    return true;
  });
    ...
    switch (confirmModal.action) {
      ...
      case 'transfer':
        return {
          ...
          icon: <ArrowRightLeft className="w-6 h-6 text-amber-600 dark:text-amber-400" />,
        };
      case 'delete':
        return {
      ...
      case 'editSpecs':
        return {
          ...
          icon: <Settings className="w-6 h-6 text-amber-600 dark:text-amber-400" />,
        };
      case 'printLabel':
        return {
      ...
      case 'changeCustodian':
        return {
          ...
          icon: <UserCog className="w-6 h-6 text-amber-600 dark:text-amber-400" />,
        };
      default:
        return {
          title: '',
    ...
    <div className="space-y-4">
      ...
      {!loading && !error && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Category Filter */}
          <div>
            <label className="text-xs text-gray-500 dark:text-gray-500 mb-2 block">Categoría</label>
            <select
          ...
          {/* Status Filter */}
          <div>
            <label className="text-xs text-gray-500 dark:text-gray-500 mb-2 block">Estado Operativo</label>
            <select
              value={statusFilter}
      ...
      <div className="space-y-3">
        {filteredAssets.map((asset, index) => {
          ...
          const CategoryIcon = categoryIcons[asset.categoria_icon as keyof typeof categoryIcons] || Package2;
              ...
              <div className="p-6">
                ...
                >
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-5 gap-4 items-start">
                    <div>
                      <p className="text-xs text-gray-500 dark:text-gray-500 mb-1">Código Etiqueta</p>
                      <p className="font-bold text-gray-900 dark:text-white">{asset.codigo_etiqueta}</p>
                      <p className="text-sm text-gray-700 dark:text-gray-300 mt-1">{asset.nombre || asset.especificaciones.modelo || 'Sin nombre'}</p>
                      <p className="text-xs text-gray-400 dark:text-gray-600 mt-1">{asset.categoria?.nombre}</p>
                    </div>
                    
                    <div className="md:col-span-2">
                      <p className="text-xs text-gray-500 dark:text-gray-500 mb-1">Ubicación (Jerarquía Institucional)</p>
                      <div className="flex items-start gap-1.5 text-gray-700 dark:text-gray-300">
                        ...
                        <p className="text-xs font-medium">{asset.ubicacion_texto}</p>
                      </div>
                    </div>
                    
                    <div>
                      <p className="text-xs text-gray-500 dark:text-gray-500 mb-1">Custodio Actual</p>
                      <div className="flex items-start gap-1.5 text-gray-700 dark:text-gray-300">
                        <User className="w-3.5 h-3.5 mt-0.5" />
                        <div>
                          <p className="text-sm font-medium">{asset.custodio_actual?.nombre_completo || 'Sin asignar'}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-500">{asset.custodio_actual?.departamento}</p>
                        </div>
                      </div>
                    </div>

                    <div>
                      <p className="text-xs text-gray-500 dark:text-gray-500 mb-1">Estado</p>
                      <span
                        className={`inline-block px-4 py-2 rounded-full text-sm font-medium border ${
                          statusColors[asset.estado_color as keyof typeof statusColors]
                        }`}
                      >
                        {asset.estado_nombre}
                      </span>
                  ...
                  >
                    {asset.especificaciones && Object.keys(asset.especificaciones).length > 0 ? (
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                          {Object.entries(asset.especificaciones).map(([key, value]) => (
                            ...
                            >
                              ...
                              <span className="text-xs text-gray-600 dark:text-gray-400">{value as string}</span>
                            </motion.div>
                          ))}
                        </div>
                      </>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-200 dark:border-gray-700">
                          <p className="text-xs text-gray-500 dark:text-gray-500 mb-1">Categoría</p>
                          <p className="text-sm text-gray-900 dark:text-white font-medium">{asset.categoria?.nombre}</p>
                        </div>
                        <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-200 dark:border-gray-700">
                          <p className="text-xs text-gray-500 dark:text-gray-500 mb-1">Estado Operativo</p>
                          <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium border ${statusColors[asset.estado_color as keyof typeof statusColors]}`}>
                            {asset.estado_nombre}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div className="p-4 bg-emerald-50 dark:bg-emerald-500/10 rounded-2xl border border-emerald-200 dark:border-emerald-700/30">
                          ...
                          <p className="text-lg font-bold text-emerald-700 dark:text-emerald-400">
                            ${asset.datos_financieros.costo_adquisicion.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                          </p>
                        </div>
                        <div className="p-4 bg-blue-50 dark:bg-blue-500/10 rounded-2xl border border-blue-200 dark:border-blue-700/30">
                          <p className="text-xs text-blue-600 dark:text-blue-400 mb-1">Valor en Libros</p>
                          <p className="text-lg font-bold text-blue-700 dark:text-blue-400">
                            ${asset.datos_financieros.valor_libro_actual.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                          </p>
                        </div>
                        <div className={`p-4 rounded-2xl border ${
                          asset.dias_hasta_vencimiento_garantia && asset.dias_hasta_vencimiento_garantia < 30
                    ...
                    <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
                      <p className="text-xs text-gray-500 dark:text-gray-500 mb-3">Acciones Disponibles</p>
                      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        <motion.button
                          whileHover={{ scale: 1.02, y: -2 }}
                          whileTap={{ scale: 0.98 }}
                          onClick={() => setConfirmModal({ isOpen: true, action: 'edit', assetId: asset.id, assetName: asset.nombre })}
                        ...
                        >
                          ...
                          <span className="text-xs font-medium text-gray-900 dark:text-white">Editar Activo</span>
                        </motion.button>

                        <motion.button
                          whileHover={{ scale: 1.02, y: -2 }}
                          whileTap={{ scale: 0.98 }}
                          onClick={() => setConfirmModal({ isOpen: true, action: 'transfer', assetId: asset.id, assetName: asset.nombre })}
                          className="px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-black dark:hover:border-white transition-colors flex flex-col items-center gap-2"
                        >
                          <ArrowRightLeft className="w-5 h-5 text-gray-700 dark:text-gray-300" />
                          <span className="text-xs font-medium text-gray-900 dark:text-white">Transferir</span>
                        </motion.button>

                        <motion.button
                          whileHover={{ scale: 1.02, y: -2 }}
                          whileTap={{ scale: 0.98 }}
                          onClick={() => setConfirmModal({ isOpen: true, action: 'changeCustodian', assetId: asset.id, assetName: asset.nombre })}
                          className="px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-black dark:hover:border-white transition-colors flex flex-col items-center gap-2"
                        >
                          <UserCog className="w-5 h-5 text-gray-700 dark:text-gray-300" />
                          <span className="text-xs font-medium text-gray-900 dark:text-white">Cambiar Custodio</span>
                        </motion.button>

                        <motion.button
                          whileHover={{ scale: 1.02, y: -2 }}
                          whileTap={{ scale: 0.98 }}
                          onClick={() => setConfirmModal({ isOpen: true, action: 'editSpecs', assetId: asset.id, assetName: asset.nombre })}
                          className="px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-black dark:hover:border-white transition-colors flex flex-col items-center gap-2"
                        >
                          <Settings className="w-5 h-5 text-gray-700 dark:text-gray-300" />
                          <span className="text-xs font-medium text-gray-900 dark:text-white">Editar Specs</span>
                        </motion.button>

                        <motion.button
                          whileHover={{ scale: 1.02, y: -2 }}
                          whileTap={{ scale: 0.98 }}
                          onClick={() => setConfirmModal({ isOpen: true, action: 'printLabel', assetId: asset.id, assetName: asset.nombre })}
                          className="px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-black dark:hover:border-white transition-colors flex flex-col items-center gap-2"
                        >
                          <FileText className="w-5 h-5 text-gray-700 dark:text-gray-300" />
                          <span className="text-xs font-medium text-gray-900 dark:text-white">Imprimir Etiqueta</span>
                        </motion.button>

                        <motion.button
                          whileHover={{ scale: 1.02, y: -2 }}
                          whileTap={{ scale: 0.98 }}
                          onClick={() => setConfirmModal({ isOpen: true, action: 'delete', assetId: asset.id, assetName: asset.nombre })}
                        ...
                        >
                          ...
                          <span className="text-xs font-medium text-red-900 dark:text-red-400">Eliminar</span>
                        </motion.button>
                      </div>
                    </div>
                  </motion.div>
