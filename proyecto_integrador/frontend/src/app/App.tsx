...
type ViewType = 'login' | 'dashboard' | 'inventario' | 'auditorias' | 'registro-auditorias' | 'alertas' | 'proveedores' | 'usuarios' | 'assetDetail' | 'auditDetail' | 'userDetail' | 'proveedorDetail' | 'settings';

export default function App() {
  const [currentView, setCurrentView] = useState<ViewType>('login');
  const [selectedAssetId, setSelectedAssetId] = useState<string | null>(null);
  const [selectedAuditId, setSelectedAuditId] = useState<string | null>(null);
  const [auditType, setAuditType] = useState<'scheduled' | 'completed'>('scheduled');
  ...
  return (
    <ThemeProvider>
      <div className="min-h-screen bg-[#F3F4F6] dark:bg-[#0a0a0a] relative transition-colors duration-300">
        <FloatingSidebar 
          activeView={
            currentView === 'assetDetail' ? 'inventario' : 
            currentView === 'auditDetail' ? 'auditorias' : 
            currentView === 'userDetail' ? 'usuarios' :
            currentView === 'proveedorDetail' ? 'proveedores' :
            currentView
          } 
          onNavigate={handleNavigate} 
        />
        <FloatingHeader onSettingsClick={handleSettingsClick} />
        ...
        {currentView === 'proveedores' && <DirectorioProveedores onProveedorClick={handleProveedorClick} />}

        {/* View 7: Gestión de Usuarios */}
        {currentView === 'usuarios' && <GestionUsuarios onUserClick={handleUserClick} />}
